    ...
    def check_inputs(
        ...
        if padding_mask_crop is not None:
            ...
            if isinstance(mask_image, list):
                for i, mask in enumerate(mask_image):
                    if not isinstance(mask, PIL.Image.Image):
                        raise ValueError(
                            f"The mask image at index {i} should be a PIL image when inpainting mask crop, but is of type"
                            f" {type(mask)}."
                        )
            elif not isinstance(mask_image, PIL.Image.Image):
                raise ValueError(
    ...
    def batch_inference(
        ...
        **kwargs,
        ...
        Args:
            ...
            image (`PIL.Image.Image`):
                `Image`, or tensor representing an image batch which will be inpainted, *i.e.* parts of the image will
                be masked out with `mask_images` and repainted according to `prompt`.
            mask_images (`List[PIL.Image.Image]`):
                `Image`, or tensor representing an image batch, to mask `image`. White pixels in the mask will be
                repainted, while black pixels will be preserved. If `mask_image` is a PIL image, it will be converted
                to a single channel (luminance) before use. If it's a tensor, it should contain one color channel (L)
                instead of 3, so the expected shape would be `(B, H, W, 1)`.
        ...
        if padding_mask_crop is not None:
            ...
            for mask_image in mask_images:
                crop_coord = self.mask_processor.get_crop_region(mask_image, width, height, pad=padding_mask_crop)
                crops_coords.append(crop_coord)
            resize_mode = "fill"
        else:
            crops_coords = None
            resize_mode = "default"
        ...
        if padding_mask_crop is not None:
            image = [self.image_processor.apply_overlay(mask_images, original_image, i, crops_coords) for i in image]
